...
void eval(char *cmdline) 
{
	...
	state = bg+1; /* foreground: 1, background: 2 */
	if(argv[0]==NULL) return;
	if(!builtin_cmd(argv)){
		//mask_chld를 block에 추가
...
void do_bgfg(char **argv) 
{
    ...
    if (is_job_id) { // if JID (For example, %321)
		job = getjobjid(jobs, atoi(&argv[1][1]));          // Get JID. pointer starts from the second character of second argument
        if (job == NULL) {                                 // Checks if the given JID is alive
            printf("%s: No such job\n", argv[1]);
            return;
        }
    }else{        // if PID (For example, 512)
        job = getjobpid(jobs, (pid_t) atoi(argv[1]));      // Get PID with the second argument
        if (job == NULL) {                                 // Checks if the given PID is there
	...
	/* bg */
	 if(strcmp(argv[0], "bg") == 0) {
        job->state = BG;                                            // Change (FG > BG) or (ST ->  BG)
	...
	/* fg */
        ...
        //st state인 job에 대해 SIGCONT signal 주고
		kill(-(job->pid), SIGCONT);                              // Send SIGCONT signal to entire group of the given job
		//state가 bg인 job에 대해 마칠때까지 실행
        waitfg(job->pid);                                           // Wait for fg job to finish
    }
